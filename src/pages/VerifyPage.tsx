    ...
    <div className="min-h-screen flex flex-col">
      ...
      {hasAttestation ? (
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          {/* Left panel — scrollable verification sections */}
          <div className="lg:w-[480px] shrink-0 overflow-y-auto border-r order-2 lg:order-1">
            <SimpleVerificationView
              job={job}
              verificationResult={verificationResult}
              activeSection={activeGraphSection}
          ...
          {/* Right panel — graph + detail view (Phala-style) */}
          <div className="flex-1 min-w-0 flex flex-col order-1 lg:order-2">
            {/* Nodes graph — top portion */}
            <div className="h-[300px] lg:h-[50%] shrink-0 border-b">
              <TrustFlowGraph
            ...
            {/* Detail view — bottom portion, scrollable */}
            <div className="flex-1 overflow-y-auto">
              <RightDetailPanel
                sectionId={activeGraphSection}
                job={job}
                verificationResult={verificationResult}
              />
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          {/* Expandable verification cards (fallback for no-attestation) */}
          <div className="space-y-3">
            {sections.map((section) => {
              const status = getStepStatus(section.id, verificationResult);
...
function RightDetailPanel({
  ...
  const renderContent = () => {
    switch (sectionId) {
      case "hardware":
        return <HardwareDetail job={job} result={verificationResult} />;
      case "enclave-image":
